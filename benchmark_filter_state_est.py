...
def experiment(args):
    ...
    fname = './data/' + method.__name__ + '/' + method.__name__ + '_filter_l96_state_benchmark_seed_' +\
            ...
            str(obs_un).zfill(3) + '_nanl_' + str(nanl).zfill(3) + '_tanl_' + str(tanl).zfill(3) + \
            '_N_ens_' + str(N_ens).zfill(3) + '_inflation_' + str(np.around(infl, 2)).zfill(4) + '.txt'
...
experiment([fname, ienkf, 0, 1.0, 40, 14, 1.18])

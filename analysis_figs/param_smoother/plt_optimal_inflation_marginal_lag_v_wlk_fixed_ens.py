...
for j in range(len(versions)):
    for i in range(len(methods)):
        ...
        for k in range(len(wlks)):
            ...
            rmse_list.append(np.mean(data[method + '_' + stat + '_rmse'][:, ens - 14]))
            spread_list.append(np.mean(data[method + '_' + stat + '_spread'][:, ens - 14]))
...
plt.figtext(.50, .03, r'Parameter walk std', horizontalalignment='center', verticalalignment='center', fontsize=24)
plt.figtext(.50, .88, r'Optimally tuned inflation, marginalized lag, ensemble size ' + str(ens), 

...
from ensemble_kalman_schemes import lag_shift_smoother_classical, lag_shift_smoother_hybrid 
import pickle
import copy
import sys
import ipdb
...
def experiment(args):
    ...
    for i in range(lag, nanl + 2 * lag + 1, shift):
        ...
        for j in range(shift):
            ...
            ipdb.set_trace()
            print(fore_rmse[i - shift + j + 1], fore_spread[i - shift + j + 1])
            print(filt_rmse[i - shift + j + 1], filt_spread[i - shift + j + 1])
            print(anal_rmse[i - lag + j], anal_spread[i - lag + j])
    ...
    fname = './data/' + method + '/' + method + '_smoother_l96_state_benchmark_seed_' +\
...
# [time_series, method, seed, lag, shift, obs_un, obs_dim, N_ens, infl] = args
experiment([fname, 'etks', 0, 2, 1, 1.0, 40, 20, 1.05])
